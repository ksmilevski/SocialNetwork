class Validator {
	...
	validate(e) {
		...
		if(elFields[fieldName].required) {
			if(fieldValue === '') {
				this.errors[fieldName].push('Empty field!');
			}
		}

		if(elFields[fieldName].email) {
			if(!this.validateEmail(fieldValue)) {
				this.errors[fieldName].push('Email not valid!');
			}
		}

		if(fieldValue.length < elFields[fieldName].minlength || fieldValue.length > elFields[fieldName].maxlength) {
			this.errors[fieldName].push(`Field must have minimum ${elFields[fieldName].minlength} and maxiumum  ${elFields[fieldName].maxlength} characters!`);
		}
		...
		if(elFields[fieldName].matching) {
			...
			if(fieldValue !== matchingEl.value) {
				this.errors[fieldName].push('Password not matching!');
			}
